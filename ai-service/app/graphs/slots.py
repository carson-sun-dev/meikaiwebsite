...
SLOT_PROMPT = """从用户消息中提取装修报价相关字段,严格输出 JSON,不要解释、不要 markdown 围栏。

已知业务线:{bl_zh}

字段说明:
- is_quote_intent: 用户是否在问价格/报价/预算/多少钱(boolean)
- area_sqm: 面积平方米(数字 或 null)。"80平"/"100㎡"/"两百平米"/"90 个平方"都要转成数字
- business_type: 业态细分(仅门面房有意义)。如:火锅店/饭店/茶馆/咖啡店/眼镜店/服装店/便利店/美容院;无法判断填 null

示例输出:{{"is_quote_intent": true, "area_sqm": 200, "business_type": "火锅店"}}
...
def _heuristic_extract(user_msg: str) -> dict[str, Any]:
    ...
    # WHY 没命中关键词但有"面积+业态"也算报价意图(用户"我开个火锅店 200 平"显然在问价)
